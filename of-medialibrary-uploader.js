...
/*-----------------------------------------------------------------------------------*/
    ...
    removeFile: function () {
     ...
     $('.mlu_remove_button').live('click', function(event) { 
        var image_to_remove_test = $(this).parent().siblings('.screenshot').find('a');
        image_to_remove_test.fadeOut(500,function(){ $(this).remove(); });
        $(this).fadeOut();
...
/*-----------------------------------------------------------------------------------*/
  ...
  var formfield,
    ...
    tb_show( woo_title, 'media-upload.php?post_id=0&TB_iframe=1' );
    return false;
  });
  ...
  window.send_to_editor = function(html) {
    ...
    if (formfield) {
      ...
      var htmlBits = html.split("'"); // jQuery seems to strip out XHTML when assigning the string to an object. Use alternate method.
          ...
          $(target).siblings('.screenshot').fadeIn().html(btnContent);
          clickedObject.siblings('.mlu_remove_button').fadeIn();
          tb_remove();
