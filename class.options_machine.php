...
class Options_Machine {
  ...
  /**
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				//sidebar_generator
				case 'sidebar_generator':
					$_id = strip_tags( strtolower($value['id']) );
					$output .= 
						'<table class="sidebar_generator_table" id="'.$value['id'].'">
						<tr class="t_head">
						<th>Sidebar Name</th>
						<th class="sidebar_mid">CSS class</th>
						<th class="sidebar_last">Options</th>
						</tr>
						<tr class="no_sidebar">
						<td colspan="3">
						<p>there are NO any sidebars in database PLZ insert at least one</p>
						</td>
						</tr>';
					...
					$output .=
						'</table><!-- end table -->
						<div class="button add_new_sidebar">
						Add New Sidebar
						</div><!-- end add_new_sidebar -->';
	...
	public static function optionsframework_sidebar_function($id,$std,$oldorder,$order){
	    ...
	    if (isset($sidebar[$oldorder])) {
	    	$val = $sidebar[$oldorder];
	    } else {
	    	$val = $std;
	    }
		
		//initialize all vars
		$sidebarvars = array('sidebar_name','sidebar_class');
		...
		if (!empty($val['sidebar_name'])) {
			$table .=
				'<td>
					<p class="sidebar_name">'. stripslashes($val['sidebar_name']) .'</p>
					<input class="sidebar_name"  type="text" name="'. $id .'['.$order.'][sidebar_name]" id="'. $id .'_'.$order .'_sidebar_name" value="'. stripslashes($val['sidebar_name']) .'" />
				</td>';
		} else {
			$table .=
				'<td>
					<p class="sidebar_name">sidebar '.$order.'</p>
					<input class="sidebar_name"  type="text" name="" value="sidebar '.$order.'" />
				</td>';
		}


		if (!empty($val['sidebar_class'])) {
			$table .=
				'<td>
					<p class="sidebar_name">'. stripslashes($val['sidebar_class']) .'</p>
					<input class="sidebar_name"  type="text" name="'. $id .'['.$order.'][sidebar_class]" id="'. $id .'_'.$order .'_sidebar_class" value="'. stripslashes($val['sidebar_class']) .'" />
				</td>';
		} else {
			$table .=
				'<td>
					<p class="sidebar_name">hema</p>
					<input class="sidebar_name"  type="text" name="" value="sidebar '.$order.' class" />
				</td>';
		}
		...
		$table .= 
			...
			<td class="hidden">
					<input type="hidden" class="order" name="'. $id .'['.$order.'][order]" id="'. $id.'_'.$order .'_sidebar_order" value="'.$order.'" />
			</td>
			</tr>';
