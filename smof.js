...
jQuery(document).ready(function($){
  ...
  var last_tr = jQuery('table.sidebar_generator_table').find('tr:last').index();
	if (last_tr > 1) {
		jQuery('table.sidebar_generator_table').find('tr.no_sidebar').addClass('hidden');
	};

	//Add new slide
	jQuery("div.add_new_sidebar").live('click', function(){		
		var sidebarsContainer = jQuery(this).prev();
		var sidebarId = sidebarsContainer.attr('id');
		var side_numArr = jQuery('#'+ sidebarId +' tr').find('input.order').map(function() {
			var str = this.id;
			str = str.replace(/\D/g,'');
			str = parseFloat(str);
			return str;
		...
		var side_maxNum = Math.max.apply(Math, side_numArr);
		if (side_maxNum < 1 ) { side_maxNum = 0};
		var side_newNum = side_maxNum + 1;
		
		var newsidebar = jQuery('<tr>\
			<td>\
				<p class="sidebar_name">sidebar ' + side_newNum + '</p>\
				<input class="sidebar_name" type="text" name="' + sidebarId + '[' + side_newNum + '][sidebar_name]" id="' + sidebarId + '_' + side_newNum + '_sidebar_name" value="sidebar ' + side_newNum + '" />\
			</td>\
			<td class="sidebar_mid">\
				<p class="sidebar_class">sidebar ' + side_newNum + ' class</p>\
				<input class="sidebar_class" type="text" name="' + sidebarId + '[' + side_newNum + '][sidebar_class]" id="' + sidebarId + '_' + side_newNum + '_sidebar_class" value="sidebar ' + side_newNum + ' class" />\
			</td>\
			<td class="sidebar_last">\
				<div class="sidebar_button remove_sidebar" title="Delete Sidebar"></div>\
				<div class="sidebar_button edit_sidebar" title="Edit Sidebar"></div>\
				<div class="sidebar_button cancel_sidebar" title="Cancel Changes"></div>\
				<div class="sidebar_button save_sidebar" title="Save Changes"></div>\
			</td>\
			<td class="hidden">\
				<input type="hidden" class="order" name="' + sidebarId + '[' + side_newNum + '][order]" id="' + sidebarId + '_sidebar_order-' + side_newNum + '" value="' + side_newNum + '">\
			</td>\
		</tr>');
	...
	// remove sidebar
	jQuery('table.sidebar_generator_table').find('div.remove_sidebar').live('click', function(){
		last_tr = jQuery('table.sidebar_generator_table').find('tr:last').index();
		if (last_tr <= 2) {
			jQuery('table.sidebar_generator_table').find('tr.no_sidebar').removeClass('hidden');
		};
		jQuery('table.sidebar_generator_table').find('tr').has(this).remove();
	});

	// edit sidebar
	jQuery('table.sidebar_generator_table').find('div.edit_sidebar').live('click', function(){
		edit_input = jQuery('table.sidebar_generator_table').find('tr').has(this);
		jQuery(edit_input).addClass('edit_current');
		jQuery(edit_input).find('input').css('display','block');
		jQuery(edit_input).find('div.remove_sidebar, div.edit_sidebar').css('display','none');
		jQuery(edit_input).find('div.cancel_sidebar, div.save_sidebar').css('display','block');
	});

	// sidebar reset
	function sidebar_reset(){
		jQuery(edit_input).removeClass('edit_current');
		jQuery(edit_input).find('input').css('display','none');
		jQuery(edit_input).find('div.remove_sidebar, div.edit_sidebar').css('display','block');
		jQuery(edit_input).find('div.cancel_sidebar, div.save_sidebar').css('display','none');
	};
	// save sidebar
	jQuery('table.sidebar_generator_table').find('div.save_sidebar').live('click', function(){
		edit_input = jQuery('table.sidebar_generator_table').find('tr').has(this);
		var sidebar_name  = jQuery(edit_input).find('input.sidebar_name').val();
		var sidebar_class = jQuery(edit_input).find('input.sidebar_class').val();
		jQuery(edit_input).find('td p.sidebar_name').html(sidebar_name);
		jQuery(edit_input).find('td p.sidebar_class').html(sidebar_class);
		sidebar_reset();
	});

	// cancel siedebar
	jQuery('table.sidebar_generator_table').find('div.cancel_sidebar').live('click', function(){
		edit_input = jQuery('table.sidebar_generator_table').find('tr').has(this);
		var sidebar_name  = jQuery(edit_input).find('td p.sidebar_name').html();
		var sidebar_class = jQuery(edit_input).find('td p.sidebar_class').html();
		jQuery(edit_input).find('input.sidebar_name').val(sidebar_name);
		jQuery(edit_input).find('input.sidebar_class').val(sidebar_class);
		sidebar_reset();
	});
